...
class PaymentsController < ApplicationController
  ...
  def import
    ...
    if !uploaded_io || uploaded_io.tempfile.class != Tempfile
      ...
      json_response({error: "no timezone or error format eg: '+03:00' "}, 400)
    else
  ...
  def archive
    if search_params[:archive_before] && Time.zone.parse(search_params[:archive_before]) < Time.zone.now - CONFIG[:archive_limit].days
      ...
      json_response( {error: "Please select a date and only record which is #{CONFIG[:archive_limit]} days before can be archived"}, 400 )
    end
  end
